...
# Calculate VALUE outcome averages ----------------------------
...
all_eng_ps <- all_eng %>% select(PS1.1, PS2.1, PS3.1, PS4.1, PS5.1,PS6.1) %>% rowMeans( . , na.rm = TRUE)
all_eng_ct <- all_eng %>% select(CT1.1, CT2.1, CT3.1, CT4.1, CT5.1) %>% rowMeans( . , na.rm = TRUE)
all_eng_wc <- all_eng %>% select(WC1.1, WC2.1, WC3.1, WC4.1, WC5.1) %>% rowMeans( . , na.rm = TRUE)
